...
const List = () => {
  ...
  return (
 <div className="pt-1 bg-gray-50 min-h-screen px-4">
  {/* Header */}
  <div className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4 ${openForm ? 'md:w-3/4' : 'w-full'}`}>
...
>
  {isChecked && <Check size={16} strokeWidth={3} className="text-gray-500" />}
                        ...
                        />
                        <span onClick={() => navigate(`/members/update?id=${user.id}`)}>{user.firstName || '-'}</span>
                      </div>
                    </td>
        ...
        <p className="text-center p-4 border-dotted border rounded-md bg-white">No Members Found</p>
      )}
    </div>
